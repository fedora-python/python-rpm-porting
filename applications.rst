...
**In essense, porting of an application RPM mostly consists of going through the spec file and adding the number 3 or substituting number 3 for number 2 where appropriate. Occasionally also substituting old macros for the new ones that are more versatile.**
...
For example, if you find the ``%py2_install`` macro, substitute with ``%py3_install``, if you find the ``%{__python2}`` macro, substitute with ``%{__python3}``, etc.
